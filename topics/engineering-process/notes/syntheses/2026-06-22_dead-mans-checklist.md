...
| **KB / Quartz not updating with new notes** | The laptop (which pushed to the firebat bare repo) is gone; `kb-relink`/`kb-lint`/`kb-incoming-refresh` still run on firebat but no new content arrives | Expected. The **org mirror of `markkb` is now the source of truth** (once WS-B lands). Point workflows at it. |
