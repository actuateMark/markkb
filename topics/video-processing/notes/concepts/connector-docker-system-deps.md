...
| [[ffmpeg-entity|FFmpeg]] TLS for `https://`/`rtsps://` | GnuTLS or OpenSSL | ⚠ depends on [[pyav-entity|PyAV]] wheel's bundled [[ffmpeg-entity|FFmpeg]] config (typically GnuTLS) | ✅ `--enable-gnutls` not set in GPU configure (see line 85+) — **SEE GAP** | ✅ `--enable-gnutls` in `build_ffmpeg.sh:54` | ⚠ GPU configure does **not** include `--enable-gnutls` — **SEE GAP** |
