...
| `exacq` | `exacq/exacq_factory.py` | `ExacqCamera` | `AvUrlFramePuller` (video stream) **or** `JpgFrameQueuePuller` (JPEG stream) | [[pyav-entity|PyAV]] **or** [[opencv-entity|OpenCV]] `cv2.imdecode` | [[rtsp-deep-dive|RTSP]] **or** HTTP [[mjpeg-and-still-image-formats|MJPEG]] | Yes ([[pyav-entity|PyAV]] path only) | Mixed; JPEG path unavoidable |
| `openeye` | `openeye/openeye_factory.py` | `OpeneyeCamera` | `AvUrlFramePuller` / `MotionBasedAvUrlFramePuller` | [[pyav-entity|PyAV]] | [[rtsp-deep-dive|RTSP]] | Yes | **Migrated** |
| `orchid` | `orchid/orchid_factory.py` | `OrchidCamera` | `OrchidJpgFrameQueuePuller` | `requests` HTTP + [[opencv-entity|OpenCV]] `cv2.imdecode` | HTTP JPEG polling | No | Permanent — Orchid offers no [[rtsp-deep-dive|RTSP]] route from us |
| `star4live` | `star4live/star4live_factory.py` | `Star4LiveCamera` | `SocketPuller` | [[opencv-entity|OpenCV]] `cv2.VideoCapture` on Unix-pipe | Unix domain socket from Star4Live SDK sidecar | No ([[opencv-entity|cv2]] software) | Bespoke — sidecar feeds raw |
...
| `eagle_eye` / `eagle_eye_v3` | `eagle_eye/eagle_eye_factory.py` | `EagleEyeCamera` | `AvUrlFramePuller` / `MotionBasedAvUrlFramePuller` | [[pyav-entity|PyAV]] | HTTPS preview MP4 (Eagle Eye HTTP) | Yes | **Migrated** |
| `hikcentral` | `hikcentral/hikcentral_factory.py` | `HikcentralCamera` | `AvUrlFramePuller` (default) **or** `OnOffMotionBasedUrlFramePuller` (motion) | [[pyav-entity|PyAV]] **or** **legacy [[opencv-entity|OpenCV]] `UrlFramePuller`** | [[rtsp-deep-dive|RTSP]] | Partial (motion path is [[opencv-entity|OpenCV]]) | **Half-migrated — motion path still legacy** |
| `kvs` | `kvs/kvs_factory.py` | `KvsCamera` | `KVSFramePuller` (`KVSFramePuller`+`KvsIngestor`) | [[pyav-entity|PyAV]] (over [[kvs-components|KVS]] [[hls-and-dash|HLS]]/GetMedia output) | [[aws-kvs-entity|AWS KVS]] | Yes ([[pyav-entity|PyAV]]) | **Migrated** |
| `vch` | `autopatrol/vch_factory.py` | `VCHCamera` | `AutopatrolWebSocketStreamPuller` (subclass of `AvUrlFramePuller`) | [[pyav-entity|PyAV]] (fMP4 over WebSocket) | Immix WebSocket fMP4 (~2 s burst) | Yes | **Migrated** ([[pyav-entity|PyAV]] under WS) |
| `autopatrol` | `autopatrol/autopatrol_factory.py` | `AutoPatrolCamera` | `AutopatrolWebSocketStreamPuller` (subclass of `AvUrlFramePuller`); `WebcamFramePuller` only when `customer.local && use_webcam` | [[pyav-entity|PyAV]] | Immix WebSocket fMP4 (~10 s patrol) | Yes | **Migrated** |
| `SMTP_per_camera` / `ajax` | `smtp/smtp_factory.py` | `SMTPCamera` | `VideoQueueFramePuller` | [[opencv-entity|OpenCV]] (`cv2.VideoCapture` on staged MP4) | SMTP-attached MP4 → local file → [[opencv-entity|cv2]] | No | Batch — [[opencv-entity|OpenCV]] is acceptable for MP4 files |
