...
The VLM FP Reduction filter is a major initiative within [[settings-automation/_summary|Settings Automation (H1.4)]] that uses Vision Language Models to post-process detection alerts and filter out false positives before they reach operators. Tracked as SA-221 and PROD-2, with an MVP definition actively driven by [[laura-reno|Laura Reno]] as of April 2026.

## The False Positive Problem

Traditional object detection models (YOLO variants, Actuate's proprietary threat models) produce false positives at rates that can overwhelm operators. Common culprits include: reflections triggering person detection, animals flagged as intruders, swaying vegetation classified as motion threats, and lighting changes interpreted as fire. Each false positive erodes operator trust and contributes to alert fatigue -- the same problem [[watchman/_summary|Actuate Watchman]]'s [[triage-gamification]] system is designed to combat from the engagement side.

The VLM FP filter attacks the problem from the technical side: catch false positives before they become alerts.

## How the Filter Works

The VLM FP filter operates as a second-pass verification layer in the [[detection-pipeline|detection pipeline]]:
...
This architecture adds latency ([[vlm-inference|VLM inference]] takes time) but dramatically reduces the volume of false alerts reaching operators.

## MVP Scope (SA-221)

[[laura-reno|Laura Reno]] is defining the MVP, which includes:
...
- **[[new-relic|New Relic]] logging** -- Adequate observability to monitor VLM filter performance in production (latency, verdict distribution, error rates)
...
[[carlos-torres|Carlos Torres]] leads model routing and the VLM FP filter implementation (SA-171 epic). The VLM/LLM Version 2.0 epic (PROD-272) tracks next-generation model integration including supervisor models (PROD-273) and an upgraded Temporal Linker (PROD-274).

## Key People

| Person | Role |
|--------|------|
| [[laura-reno|Laura Reno]] | MVP definition, requirements |
| [[carlos-torres|Carlos Torres]] | Model routing, filter implementation |
| [[zack-schmidt|Zack Schmidt]] | Testing, productionisation |
| [[otzar-jaffe|Otzar Jaffe]] | Related work on [[ignore-zones|ignore zones]] and Classifyr |

## Relationship to Other Initiatives

VLM FP reduction is listed as reused technology that feeds into [[watchman/_summary|Actuate Watchman]]. In [[watchman-repo|Watchman]]'s [[multi-agent-architecture]], the filter would sit within the Actuate Threat Agent's dual-track routing -- VLM verification happens after the YOLO precursor gate and before events are forwarded to the Assessment Agent. Reducing false positives at this stage means the Assessment Agent processes fewer garbage events, the Recommendation Agent generates fewer spurious instructions, and operators see cleaner triage queues in the [[triage-gamification]] system.
