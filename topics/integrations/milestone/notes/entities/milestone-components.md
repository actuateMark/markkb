...
- **S3-token writer chain (split-brain architecture, documented 2026-05-24 during incident response):** For the four hardcoded Milestone customer paths, the read side here only DOWNLOADS from S3 — it does NOT do the SOAP `Login` itself. The SOAP `Login` and S3 PUT are done by a SEPARATE process: the K8s cronjob suite under namespace `admin-auto-onboarding`. Cronjob names: `admin-auto-onboarding-schools` (Genesis Schools / customer 14920 / `172.16.254.103`), `admin-auto-onboarding-federated` (Federated / `172.16.254.91`), `admin-auto-onboarding-gun`, `admin-auto-onboarding-group-site`. Each runs every 5 min (cron `1-59/5 * * * *`), image `admin-auto-onboarding:1.1.23`, invokes `auto_onboarding.py` → `camera_comparison.update_camera_list()` → HTTPS POST to `/ManagementServer/ServerCommandService.svc` on the customer's management server → uploads `token.txt` + `token_ttl.txt` + `token_registration_time.txt` to `s3://actuate-settings/<path>/`. **The reader (this `MilestoneService`) has no fallback** if the writer goes silent — it will keep returning the stale S3 token forever, and the recording server will reject every connection with `Security token invalid`, with no visible failure inside the connector pod. On 2026-05-24 the Schools cronjob hit `SSLEOFError(UNEXPECTED_EOF_WHILE_READING)` from the customer's management server TLS endpoint for ~10h before any human noticed — see [[2026-05-24_genesis-no-alerts-milestone-token-rejection]].
