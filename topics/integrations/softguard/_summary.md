...
- `zone` -- [[softguard-components|Softguard]] zone identifier
- `account` -- [[softguard-components|Softguard]] account number for the monitored site
- `user` and `user_ext` -- [[softguard-components|Softguard]] user identifier and extension
- `server` and `port` -- the [[softguard-components|Softguard]] receiver endpoint (from `recipient` config)
...
No explicit API authentication. The [[softguard-components|Softguard]] receiver is identified by `server` IP and `port`, and the `account`, `zone`, and `user` fields serve as the site/device identifier within the [[softguard-components|Softguard]] system. Access relies on network-level connectivity to the [[softguard-components|Softguard]] receiver.

## Alert Delivery

[[softguard-components|Softguard]] follows the standard SQS event-listener pattern: the sender enqueues to `event_queue_softguard_alarm.fifo` with `message_group_id` set to the customer ID (ensuring FIFO ordering per customer). The event-listener worker dequeues, retrieves frames from S3, and delivers the formatted alert to the [[softguard-components|Softguard]] monitoring platform.

## Architecture

The alarm sender factory in [[actuate-alarm-senders]] instantiates `SoftguardAlertSender` when a [[softguard-components|Softguard]] alert config is present. There are no Softguard-specific puller or integration calls components -- video ingestion uses whatever VMS integration ([[rtsp-deep-dive|RTSP]], Milestone, etc.) is configured, and [[softguard-components|Softguard]] is purely a send-only monitoring integration. The [[vms-connector]] builds the sender via the factory during camera initialization.
