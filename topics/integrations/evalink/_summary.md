...
Evalink is a cloud-based alarm management platform used by monitoring centers to receive, process, and dispatch security events. Actuate integrates with Evalink as an alert destination, forwarding AI-generated detections via an **SQS FIFO queue** for downstream delivery to the Evalink API.

## Components

### EvalinkAlertSender

Defined in [[actuate-alarm-senders]] at `evalink/alert_sender.py`. Extends `EventListenerAlertSender`, meaning it sends alerts through the SQS-backed event-listener pattern rather than calling the Evalink API directly. When `send()` is called with an `AlertData` object, the sender packages the alert into a message containing:
...
No Evalink-specific puller exists. Evalink is a send-only monitoring integration; video comes from whichever VMS connector the site uses.

## Auth Method

Authentication to the Evalink platform is handled at the queue-consumer level (outside actuate-libraries). At the sender level, per-site config provides the token and server details that are forwarded in the SQS message for the consumer to use.

## Key Config Fields

Alert configuration is defined in [[actuate-config]] at `alerts/evalink/alert_config.py` via `EvalinkAlertConfig`. The config reads from `evalink_alerts` in the feature deployment, collecting a list of recipient dicts. Each recipient typically includes server endpoint, authentication token, and device ID fields that the downstream consumer uses when posting to the Evalink API.

## Alert Delivery

Evalink uses the **SQS FIFO queue** pattern. The sender writes to `event_queue_evalink_alarm.fifo`, which guarantees ordered, exactly-once delivery. A separate Lambda or worker process consumes messages from this queue and forwards them to the Evalink REST endpoint. This decouples the real-time connector process from external API latency.
...
- [[vms-connector]] -- builds the sender via the alarm-sender factory when Evalink alerts are configured
