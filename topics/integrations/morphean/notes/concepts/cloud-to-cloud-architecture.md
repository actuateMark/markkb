...
Track A of the [[integrations/morphean/_summary|Morphean]] -- a platform-level integration between Actuate and Morphean's **VideoProtector REST API v2.54.0** that delivers Tier-1 analytics, [[camera-health-monitoring/_summary|Camera Health Monitoring (H1.1)]], and [[autopatrol/_summary|AutoPatrol (H1.2)]] to all of Morphean's white-label partners (most notably VIDEOR, covering 30 countries and 170+ resellers).

## How It Works

Unlike Actuate's typical deployment where camera streams come directly from on-site VMS systems via [[rtsp-deep-dive|RTSP]], the Morphean integration routes video through Morphean's cloud platform first. The data flow is:

1. **Cameras** send [[rtsp-deep-dive|RTSP]] streams to Morphean's VideoProtector cloud.
2. **Actuate ingests [[rtsp-deep-dive|RTSP]] streams** from Morphean's cloud at 480p-720p resolution and 1 FPS. This is lower resolution and frame rate than typical direct connections, optimized for bandwidth and cost.
...
| **[[rtsp-deep-dive|RTSP]] stream ingestion** | Pull camera feeds from Morphean cloud (480p-720p @ 1 FPS) |
...
| **[[ignore-zones|Ignore zones]] & sensitivity** | Actuate-side configuration (Morphean has no dedicated ROI API) |
...
A notable constraint is that **Morphean has no dedicated ROI (Region of Interest) API**, so [[ignore-zones|ignore zones]] and sensitivity must be managed entirely on the Actuate side. This means the Morphean partner UI cannot natively display or edit Actuate [[ignore-zones|ignore zones]] -- a UX gap that may need a workaround.
