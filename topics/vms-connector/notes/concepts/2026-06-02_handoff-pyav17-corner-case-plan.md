...
`CodecContext.close()` was **removed in PyAV 16** (verified against the cached **av 17.0.1** wheel: stub has `open`/`create`/`flush_buffers`, no `close`; PyAV 13.1.0 still has it). `actuate-pullers` `av_url_puller.py` calls `self._nvdec_ctx.close()` at **main lines 841 + 940** — both wrapped in `try: … except: pass`, so under PyAV 17 it's **silently swallowed** (AttributeError caught), not a crash, but the explicit cleanup never runs and the bare-except masks the removal. **#358 migrated the API surface but did not touch these.**
...
| Image | FFmpeg source | Was | Now |
|---|---|---|---|
| `x86_dockerfile` (CPU x86) | PyAV-17 wheel's bundled FFmpeg 8.1 | auto | auto (no edit) |
...
- **D1 = keep the custom FFmpeg build** (not stock wheel). Rationale: we control `--enable-gnutls` (rtsps/https — §15a shows what breaks without it), preserve the vaapi/cuda HW-accel door (§5 fleet-arch GPU/co-located inference direction), `--enable-libx265`. +5min cold build is cheap vs a TLS-capability regression.
...
4. **Build validation** — `docker buildx` ARM-CPU (PyAV-17 from source vs FFmpeg-8.1.1) + both GPU images (FFmpeg 8.1.1 + CUDA 12.1 nvcc + unpinned nv-codec-headers is the riskiest build). CI builds ARM64+x86 on PR — natural gate.
...
- ✅ **Custom FFmpeg build bump** — committed (`07bee6845`) + pushed on vms-connector `feature/pyav-17-bump-clean` (updates draft #1714).
...
2. **Build validation** — connector CI on #1714 builds ARM64 + x86 (exercises FFmpeg n8.1.1 from source on ARM-CPU); GPU images need a manual `docker buildx` (FFmpeg 8.1.1 + CUDA 12.1 nvcc + nv-codec-headers). Watch CI.
...
- **Local validation ladder run** (against #367 code on av17, connector venv via `PYTHONPATH` shadow):
  - **V1 RTSP decode parity** PASS — 60 frames, 0 corrupt, 0 zero-size, `skip_frame` both directions (stays `str` on av17). NONKEY-on-long-GOP tanks throughput → corroborates the starvation condition C1 guards against.
  - **V2 corrupt-HEVC synth** partial — av17 decodes corrupted HEVC without crash, damaged inter-frames dropped (not emitted gray). Could NOT synthetically isolate the `is_corrupt` suppress-vs-emit toggle (byte-flip breaks NAL parse; manual missing-ref decoder produced 0 frames). Confirms KB conclusion: faithful HEVC-corruption repro needs real bad-camera data → soak gate.
...
- **#1714 = PyAV 17.0.1 + FFmpeg 8.1.1 + NumPy 2.4.6 + OpenCV 4.13 + security/perf dep refresh** (cryptography 48, urllib3 2.7, requests 2.34, pillow 12.2, uvicorn 0.49, newrelic 13.1, psutil 7.2, shapely 2.1.2). **`actuate-pullers==1.20.3` stable, NO dev pins.** Body claims verified.
- **Validated:** 166 connector tests, all lib suites on numpy 1.26 *and* 2.4, ECR green. **Leg-1 A/B soak passed overnight** on `actuate-nyc-alibi-vigilant`: **−15% memory, −45% CPU**, reconnect parity, zero tracebacks. **Leg-2 (AmeriGas cust-41399, HEVC efficacy) pending.**
...
- **Superseded #1621** (old PyAV PR) and **closed test #1696** (cv2-dst A/B) during cleanup.
