...
1. `connector.py` -- Parses args, loads settings (S3 or local JSON), configures logging (async queue-based), pins thread counts for [[opencv-entity|OpenCV]]/BLAS, tunes memory allocator (`MALLOC_MMAP_THRESHOLD_`).
...
- `actuate-pullers` -- Stream decoders ([[rtsp-deep-dive|RTSP]] via [[pyav-entity|PyAV]], HTTP snapshot, SQS/SMTP)
...
Docker builds via `./run-container-local.sh`. GPU variants require NVIDIA runtime. [[rtsp-deep-dive|RTSP]] camera simulator available via `just play-camera-simulator`.
...
- **Thread pinning**: [[opencv-entity|OpenCV]]/BLAS threads capped at 1-2 to prevent CPU scaling with node capacity.
