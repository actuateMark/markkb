...
- [[flex-ignore-zones|Flex ignore zones]] -- multiple IZ presets per camera, full API + frontend + settings generation (AUTO-446, 500, 424, 493)
...
**[[watchman-repo|Watchman]] (B2B) demands:**
...
- Terminal-style UI -- agentic-first design ([[brad-murphy|Brad Murphy]])
- Mobile app shell -- iOS + Android (PROD-239)

**Shared dependencies where priority is ambiguous:**
- [[vlm-fp-reduction|VLM FP reduction]] (SA-221 / PROD-2) -- both products benefit, but [[watchman-repo|Watchman]]'s two-track routing formalizes it as an architectural requirement while AutoPatrol treats it as an enhancement
- EKS upgrade 1.32 -> 1.35 (ENG-79) and VPA fixes (ENG-78) -- both products need these, but [[watchman-repo|Watchman]]'s many-small-sites model makes them urgent
- Weapon v8 deployment (PROD-98) -- benefits both channels
- [[settings-automation/_summary|Settings Automation (H1.4)]] -- AutoPatrol needs it for reducing manual tuning; [[watchman-repo|Watchman]] needs it for zero-touch onboarding

## The AIM Signal

The [[alerts-improvements/_summary|Alerts Improvements (H1.3)]] initiative (H1.3) is the canary for bandwidth constraints. Of its 29 issues, **25 are unassigned**. AIM represents cross-cutting improvements to alert quality that benefit both go-to-market channels. Its stalled state suggests the team does not have capacity beyond the two primary product tracks. If AIM cannot get staffing, smaller-scope improvements to alert delivery, false positive reduction, and operator experience will accumulate as technical debt affecting both channels.
...
| Dimension | B2B2B (Current) | B2B ([[watchman-repo|Watchman]]) |
|---|---|---|
...
The risk of infrastructure divergence is that shared library changes affect both products but are tested against different assumptions. A performance optimization that helps large sites (raising shard size) may not matter for [[watchman-repo|Watchman]]'s small sites. A VPA fix validated on 50-camera deployments may behave differently on 4-camera pods with lower baseline resource usage.

## Bandwidth Allocation Reality

Looking at the named team members across both tracks:

- **[[brad-murphy|Brad Murphy]]**: AutoPatrol frontend (flex IZ, bulk updates) AND [[watchman-repo|Watchman]] UI
- **Tatiana**: Admin API backend AND AutoPatrol flex IZ API
- **[[mark-barbera|Mark Barbera]]**: Immix bounding boxes AND [[generic-patrol-mode|generic patrol mode]] AND EBUS v5 review queue
- **[[jacob-weiss|Jacob Weiss]]**: [[watchman-repo|Watchman]] infrastructure/security AND Jira reorganization AND ENG tickets
- **[[carlos-torres|Carlos Torres]]**: VLM FP filter (shared) AND weapon model training AND model routing
- **[[laura-reno|Laura Reno]]**: [[watchman-repo|Watchman]] document owner AND VLM MVP definition AND fire detection launch

Many engineers serve both tracks. The practical effect is context-switching overhead and the risk that neither track moves as fast as its standalone priority would suggest. The [[watchman-repo|Watchman]] PRD's "ASAP" priority designation competes directly with AutoPatrol's ~$800K revenue obligation.

## Strategic Tension

The fundamental question is sequencing. The B2B2B channel generates current revenue and has committed partner relationships (Immix's 27 alarm senders, Morphean's 30-country network). Underinvesting in it risks the revenue base. The B2B channel is the growth bet -- but [[watchman-repo|Watchman]]'s 10-20 beta site MVP target is deliberately small, designed to validate without disrupting. The tension becomes acute when shared infrastructure work (EKS upgrade, VPA fix, VLM deployment) must be prioritized: does it get done on the B2B2B timeline (steady, operational) or the [[watchman-repo|Watchman]] timeline (ASAP)?
