...
- **Watch Management Service as a fleet-singleton reconciler** (controller pattern — k8s-native). ([[2026-05-28_watch-management-service-design]])
...
- **Pipeline-vs-connector parity validation** — confirm Watchman pipeline output matches connector capability (motion blobs, frames, upstream signals); no missing inputs before backend plumbing.
- **The translator** — RunSpec.v1 → deployed pods (the deployer's domain); resolve the vestigial `customer.server_ip`/`username` fields under multi-camera RTSP.
...
- [[topics/fleet-architecture/_summary]] · [[topics/watchman/_summary]] · ENG-300 + Watchman epic
