...
tags: [proposal, fleet, hybrid, sidecar, smart-puller, detection-core, statefulset, autopatrol]
created: 2026-04-16
updated: 2026-04-16
author: kb-bot
---
...
- **Detection core locality:** StatefulSet with camera-affinity — a given camera-group maps to a specific pod. Smart pullers for that group route to that pod's same-AZ replica preferentially via [[pod-affinity-anti-affinity]] (`topology.kubernetes.io/zone` preferred-affinity so the scheduler doesn't wedge on capacity). Failover moves the camera-group to another AZ only on pod death.
...
**Sidecar detail:** the detection core is a StatefulSet with camera-affinity — one pod owns a group of 10-50 cameras. Promotion/demotion happens via site context service assignment. Cameras in a pod share resources ([[inference-pool|inference pool]], observer pool) — preserves today's efficiencies.

**Spot viability:** Smart Puller fleet is Spot-eligible (stateless; motion state is cheap to cold-start on replacement). **Detection Core is NOT Spot-safe** — StatefulSet cameras hold live tracker state, and Spot's 2-minute interruption window is tighter than the preStop drain + tracker-snapshot cycle under load. Run Detection Core on an On-Demand-only NodePool (or `karpenter.sh/do-not-disrupt` + tolerations on Spot, but that defeats the Spot discount). Alert Dispatch + Site Context Service are both Spot-eligible.
...
- **Why savings:**
  ...
  2. VPA fix — smart puller is steady-load; detection core is bursty. Splitting them into separate fleets lets VPA size each appropriately instead of averaging the bimodal load (fixes [[vpa-bimodal-workload-limitation|ENG-78 root cause]]; see [[vertical-pod-autoscaler-deep-dive]] for the in-place-vs-recreate mode decision)
...
- **StatefulSet upgrade behavior:** resolved via the `partition` field in `updateStrategy.rollingUpdate` — stage the rollout by ordinal, wait for the partition tail's tracker-snapshot propagation + `terminationGracePeriodSeconds` to complete before advancing the partition. See [[kubernetes-workload-controllers]] + [[k8s-controller-selection-guide]] for the partition mechanic. Worst case: 10% of cameras are mid-upgrade at any moment (the partition-tail window), acceptable given tracker resume from Redis snapshot within RTO.
...
- [[downstream-consumer-impact]] — **cleanest downstream story** — [[watchman-repo|Watchman]]/AutoPatrol/alert integrations see no contract change. Smart puller handles AutoPatrol's patrol alongside FDMD.
...
- [[infrastructure/_summary]] — adds site-context service to [[argocd|ArgoCD]] + VPA list; fewer VPA woes overall because pods are shape-consistent
...
- **Site Context Service as a broader config abstraction.** Today's `actuate-config` reads settings.json; site-context could become the standard read-path for ALL hot-path config, not just the connector. [[watchman-repo|Watchman]] and AutoPatrol could consume it too.
