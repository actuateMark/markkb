...
Flex Ignore Zones are the dominant active workstream in [[autopatrol]] as of April 2026. The feature allows multiple ignore zone (IZ) presets to be configured per camera, with different presets selectable per patrol schedule. This is a significant upgrade from the previous model where each camera had a single, static set of ignore zones.

## Problem Statement

Ignore zones mask regions of a camera's field of view where detections should be suppressed -- busy roads, swaying trees, reflective surfaces. In practice, what counts as a nuisance zone changes with context. A loading dock that should be ignored during business hours (constant legitimate movement) may need full monitoring after hours. A retail entrance that generates constant foot traffic during the day becomes a high-value intrusion zone at night.
...
- Settings generation -- when a patrol schedule activates, the correct IZ preset must be resolved and applied to the detection pipeline

### Frontend
**Brad Murphy** is building the frontend components across multiple tickets:
...
3. Passing those settings to the AI detection pipeline so that events in ignored regions are suppressed

This is the same settings generation pipeline used across Actuate's products, extended to support preset selection. The work connects to the broader [[settings-automation]] initiative.

## Relationship to Watchman

The Flex IZ concept is directly portable to [[watchman]]. Watchman's [[patrol-vs-active-modes|Patrol Agent]] inherits from AutoPatrol's scheduling infrastructure, and ignore zone handling is part of that inheritance. In Watchman's context, the [[multi-agent-architecture|Site Context Agent]] could potentially automate IZ preset switching based on learned site rhythms rather than requiring manual schedule configuration.
