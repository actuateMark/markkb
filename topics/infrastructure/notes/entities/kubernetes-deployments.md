...
A GitOps repository that manages multi-cluster Kubernetes deployments using ArgoCD's **app-of-apps** pattern. A single bootstrap Application per cluster watches this Git repo and recursively creates all child Applications, so the entire cluster state is declared in version control.

## Repository Structure

The repo has two top-level trees:

- **`argocd/`** -- ArgoCD Application definitions and per-cluster config.
  - `charts/app-of-apps/` -- the root Helm chart; creates two child apps (applications + cluster-services).
  - `charts/applications/` -- one ArgoCD Application template per business app.
  - `charts/cluster-services/` -- one ArgoCD Application template per infrastructure service.
...
Infrastructure services managed via the cluster-services chart: **ArgoCD**, **AWS Load Balancer Controller**, **AWS EBS CSI Driver**, **AWS VPC CNI**, **Karpenter** (with CRDs and NodePools), **Istio** (base, istiod, CNI, ztunnel, Grafana, Jaeger addons), **Kiali**, **cert-manager**, **external-dns**, **ingress-nginx**, **Gateway API**, **CoreDNS**, **node-local-dns**, **Prometheus stack** (with Thanos, metrics-server, Prometheus adapter), **New Relic**, **Kyverno** (with policies), **cluster-autoscaler**, **VPA**, **Spegel**, **SonarQube**, **RBAC**, **namespaces**, **Neuron** (AWS Inferentia), **NVIDIA device plugin**, **CHM node cordoner**, and **GitHub Actions Runner Controller** (ARM, x86, inference runners).

## How It Works

1. A bootstrap Application is manually applied once per cluster.
2. ArgoCD polls this repo (default ~3 min) and reconciles.
3. All values are namespaced under the `actuate:` key; cluster-values override chart defaults.
4. Three ArgoCD Projects enforce RBAC separation: `argo-cd-deployments` (bootstrap only), `argo-cd-applications` (business apps, restricted), and `argo-cd-cluster-services` (full cluster permissions).
...
2. **Deploy** -- commit a deployment manifest to this repo. Two options:
   - **Helm chart** (preferred): create an ArgoCD Application manifest with values inline, using the shared microservice chart. See `deployments/applications/internal-homepage`.
   - **Raw manifests**: write K8s YAML directly. See `deployments/applications/cost-per-site`.
3. ArgoCD reconciles automatically after merge.
