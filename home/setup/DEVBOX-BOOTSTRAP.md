...
4. **Credentials** — only what the workflow you'll use needs. Follow the consolidated **[[2026-06-24_secrets-refresh-runbook]]**: AWS SSO/CodeArtifact, `gh`, [[new-relic|New Relic]], Atlassian, Anthropic key. *(Local KB use — `/kb-ask`, `/kb-lookup`, `/daily-scope` reading — needs none of the cloud creds.)*
